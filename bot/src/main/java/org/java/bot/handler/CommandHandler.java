...
import org.java.bot.util.Message;
...
public class CommandHandler {
    ...
    public SendMessage handleSaveLink(Update update) {
        ...
        try {
            if (linkService.checkLinkFormat(link)) {
                ...
                return Message.createInvalidLinkResponse(update);
            }
        } catch (Exception e) {
    ...
    public SendMessage handleRemoveLink(Update update) {
        Long chatId = update.message().chat().id();
        Long userId = update.message().from().id();
        String link = update.message().text();
    ...
    public SendMessage handleCommandOrUnknown(Update update) {
        Command command = commands.get(update.message().text());
        return command != null && command.supports(update)
            ? command.handle(update)
            : Message.createUnknownCommandResponse(update);
    }

    public String toStringCommands() {
        return commands.gets().entrySet().stream()
